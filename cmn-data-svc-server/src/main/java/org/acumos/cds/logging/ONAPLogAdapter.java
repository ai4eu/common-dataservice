...
/**
 * Extensible adapter for cheaply meeting ONAP logging obligations using an
 * SLF4J facade.
 *
 * This can be used with any SLF4J-compatible logging provider, with appropriate
 ...
 * <li>{@link #invoke} logs and returns a UUID to passed during invocation, and
 * optionally sets these for you on your downstream request by way of an
 * adapter.</li>
 * <li>Call {@link #getServiceDescriptor()} and its setters to set
 * service-related MDCs.</li>
 * <li>Call {@link #getResponseDescriptor()} and its setters to set
 * response-related MDCs.</li>
 * </ul>
 *
 * Minimal usage is:
 * <ol>
 ...
 * </ol>
 *
 * ... if you're happy for service information to be automatically derived as
 * follows:
 * <ul>
 * <li><tt>ServiceName</tt> - from
 * <tt>HttpServletRequest#getRequestURI()</tt></li>
 * <li><tt>InstanceUUID</tt> - classloader-scope UUID.</li>
 * </ul>
 *
 * ... and if those defaults don't suit, then you can override using properties
 * on {@link #getServiceDescriptor()}, or by injecting your own adapter using
 * {@link #setServiceDescriptor(ServiceDescriptor)}, or by overriding a
 * <tt>protected</tt> methods like{@link #setEnteringMDCs}.
 *
 * For everything else:
 * <ul>
 * <li>The underlying SLF4J {@link Logger} can be retrieved using
 * {@link #unwrap}. Use this or create your own using the usual SLF4J
 * factor.</li>
...
public class ONAPLogAdapter {
	...
	public ONAPLogAdapter entering(final RequestAdapter<?> request, String format, Object... arguments) {
		...
		this.mLogger.info(ONAPLogConstants.Markers.ENTRY, format, arguments);
		return this;
	}
	 ...
	 *
	 * If you call this variant, then YOU are assuming responsibility for setting
	...
	/**
	 * Report pending invocation with <tt>INVOKE</tt> marker, setting standard ONAP
	...
	/**
	 * Header builder, which (unlike {@link RequestAdapter} will tend to vary a lot
	 * from caller to caller, since they each get to choose their own REST (or HTTP,
	 * or whatever) client APIs.
	 *
	 * No default implementation, because there's no HTTP client that's sufficiently
	 * ubiquitous to warrant incurring a mandatory dependency.
	 *
	 * @param <T>
	 *            type, for chaining.
	 */
