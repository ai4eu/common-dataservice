...
public class CdsRepositoryServiceTest {
	...
	public void testRepositories() throws Exception {
		...
		try {
			...
			// Created and modified timestamps may slightly apart
			// but note this assumes the driver fetches milliseconds,
			// which the Mariadb JDBC connector does not for type Date.
			...
			Timestamp changedCreate = new Timestamp(new Date().getTime() - 99);
			...
			for (MLPSolutionRevision r : revs) {
				...
				Assert.assertTrue(r.getAuthors().length > 0);
			...
			Timestamp beforeUpdateTs = ca.getCreated();
			...
			Page<MLPSolution> recentlyUpdated = solutionSearchService.findSolutionsByModifiedDate(true, accTypes,
					beforeUpdateTs, new PageRequest(0, 5, null));
			...
			Timestamp anHourAgo = new Timestamp(new Date().getTime() - (1000L * 60 * 60));
